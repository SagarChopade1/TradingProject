...
class AsyncTradingView(APIView):
    ...
    def post(self, request, format=None):
        ...
        file_path = default_storage.save(f'downloads/{f"candles_data_{int(time())}.json"}', ContentFile(json_data))
        return Response({"message": "File processed.", "download_url": f"{request.build_absolute_uri('/')}{settings.MEDIA_URL}downloads/{unique_filename}"})
